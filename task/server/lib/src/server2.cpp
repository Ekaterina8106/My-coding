...
json RecipeServer::filter(double minCal, double maxCal, double minTime, double maxTime) const {
    ...
    for (const auto& recipe : data) { // auto - компилятор определяет тип, элемент массива data
        try {
    ...
    ofstream output("filtered_recipes.json"); // записывает в файл
    output << result.dump(4);
    cout << "Отсортированные рецепты сохранились в filtered_recipes.json" << endl;

    return result; // по идее можно поменять на void функцию и убрать return и не будет вывода в консоль
}
...
json RecipeServer::generate() {
    static const vector<string> recipeNames = {
        "Спагетти Болоньезе", "Салат Цезарь", "Стейк",
        "Суп из грибочков", "Шоколадный тортик", "МГУ"
    };
